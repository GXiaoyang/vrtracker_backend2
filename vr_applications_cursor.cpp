...
bool VRApplicationsCursor::IsApplicationInstalled(const char * pchAppKey)
{
	...
	{
		CURSOR_SYNC_STATE(is_app_installed, applications[index].is_installed);
...
uint32_t VRApplicationsCursor::GetApplicationCount()
{
	...
	CURSOR_SYNC_STATE(active_application_indexes, active_application_indexes);
...
vr::EVRApplicationError VRApplicationsCursor::GetApplicationKeyByIndex(uint32_t unExternalApplicationIndex, char * pchAppKeyBuffer, uint32_t unAppKeyBufferLen)
{
	...
	CURSOR_SYNC_STATE(active_application_indexes, active_application_indexes);		// <-- the answer is in here
...
uint32_t VRApplicationsCursor::GetApplicationProcessId(const char * pchAppKey)
{
	...
	{
		CURSOR_SYNC_STATE(process_id, applications[internal_index].process_id);
	...
	vr::EVRApplicationError * pError)
{
	...
	{
		CURSOR_SYNC_STATE(string_prop, applications[app_index].string_props[prop_index]);
	...
	vr::EVRApplicationProperty prop_enum, vr::EVRApplicationError * pError)
{
	...
	{
		CURSOR_SYNC_STATE(bool_prop, applications[app_index].bool_props[prop_index]);
		rc = bool_prop->val;
		if (pError)
	...
	vr::EVRApplicationProperty prop_enum, vr::EVRApplicationError * pError)
{
	...
	{
		CURSOR_SYNC_STATE(uint64_prop, applications[app_index].uint64_props[prop_index]);
		rc = uint64_prop->val;
		if (pError)
...
bool VRApplicationsCursor::GetApplicationAutoLaunch(const char * pchAppKey)
{
	...
	if (GetInternalIndexForAppKey(pchAppKey, &internal_index))
	{
		CURSOR_SYNC_STATE(auto_launch, applications[internal_index].auto_launch);
	...
	char * Buffer, uint32_t Len)
{
	...
	if (GetInternalIndexForAppKey(pchAppKey, &internal_index))
	{
		CURSOR_SYNC_STATE(supported_mime_types, applications[internal_index].supported_mime_types);
...
vr::EVRApplicationError VRApplicationsCursor::GetStartingApplication(char * pchAppKeyBuffer, uint32_t unAppKeyBufferLen)
{
	...
	vr::EVRApplicationError rc;
	CURSOR_SYNC_STATE(starting_application, starting_application);
...
vr::EVRApplicationTransitionState VRApplicationsCursor::GetTransitionState()
{
	...
	CURSOR_SYNC_STATE(transition_state, transition_state);
...
bool VRApplicationsCursor::IsQuitUserPromptRequested()
{
	...
	bool rc;
	CURSOR_SYNC_STATE(is_quit_user_prompt, is_quit_user_prompt);
...
uint32_t VRApplicationsCursor::GetCurrentSceneProcessId()
{
	...
	uint32_t rc;
	CURSOR_SYNC_STATE(current_scene_process_id, current_scene_process_id);
